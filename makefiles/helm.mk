...
load-existing-images: ## Load existing Docker images to Kind cluster
	@echo "Loading existing Docker images to Kind cluster..."
	@for service in $(SERVICES) frontend; do \
		echo "Loading localhost:5001/$$service:latest..."; \
		kind load docker-image localhost:5001/$$service:latest --name wealist 2>/dev/null || echo "⚠️  Image localhost:5001/$$service:latest not found"; \
	done
	@echo "Loading infrastructure images..."
	@kind load docker-image localhost:5001/postgres:15-alpine --name wealist 2>/dev/null || true
	@kind load docker-image localhost:5001/redis:7-alpine --name wealist 2>/dev/null || true
	@kind load docker-image localhost:5001/coturn:4.6 --name wealist 2>/dev/null || true
	@kind load docker-image localhost:5001/livekit:v1.5 --name wealist 2>/dev/null || true
	@echo "✅ All existing images loaded to Kind cluster!"

quick-deploy: load-existing-images ## Quick deploy with existing images
